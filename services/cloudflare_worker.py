...
Worker может использовать разные модели: nemotron-3-120b-a12b для основных функций,
hermes-2-pro-mistral-7b для диалогового ассистента.
"""
import aiohttp
import os
import logging
...
# Базовый промпт для worker (используется и в диалоговом режиме)
DEFAULT_SYSTEM_PROMPT = (
    "Ты — Джарвис, дружелюбный помощник по питанию и здоровому образу жизни, "
    "а также эксперт по боту NutriBuddy. Твоя задача — отвечать на вопросы пользователя "
    "о питании, здоровье, тренировках, а также подробно объяснять, как работает бот, "
    "откуда берутся цифры и как пользоваться всеми его функциями.\n\n"
    "ВАЖНО: Когда пользователь спрашивает о том, как что-то считается или работает в боте "
    "(например, 'как считаются калории', 'как определяется норма', 'откуда берутся цифры'), "
    "ты должен отвечать, основываясь на описании функционала бота, а не давать общие советы "
    "из интернета. Используй информацию ниже.\n\n"
    ...
    "   - Можно отправить фото еды: бот использует Cloudflare AI (модель Llama 3.2 Vision) для распознавания блюда и ингредиентов. Результат переводится на русский, ищется в локальной базе продуктов, затем запускается интерфейс для указания веса.\n"
    ...
    "   - Можно создать напоминание о приёме пищи, воде, взвешивании или своём.\n"
...
async def ask_worker_ai(
    ...
    model: str = "@cf/hermes-2-pro-mistral-7b",  # По умолчанию hermes для диалогового ассистента
