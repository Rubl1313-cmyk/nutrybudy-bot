...
# Справочник средних весов продуктов в граммах
UNIT_WEIGHTS = {
    # Фрукты
    "яблоко": 150,
    "банан": 120,
...
def convert_to_grams(name: str, quantity: float, unit: str) -> float:
    ...
    try:
        ...
        # Если в штуках - ищем средний вес
            ...
            logger.warning(f"🔄 Неизвестный продукт для конвертации: {name}, используем 100г за шт")
            return float(quantity) * 100
        
        # Ложки
        elif unit in ['ложка', 'ложки', 'ложек', 'столовая ложка', 'столовые ложки']:
            return float(quantity) * 15
        
        elif unit in ['чайная ложка', 'чайные ложки']:
        ...
        # Пачки
        elif unit in ['пачка', 'пачки', 'пачек']:
            return float(quantity) * 200
        ...
        # Если неизвестная единица, возвращаем как есть (предположим граммы)
            ...
            logger.warning(f"🔄 Неизвестная единица измерения: {unit}, используем как граммы")
            return float(quantity)
            
    except (ValueError, TypeError) as e:
        logger.error(f"🔄 Ошибка конвертации {name} {quantity} {unit}: {e}")
